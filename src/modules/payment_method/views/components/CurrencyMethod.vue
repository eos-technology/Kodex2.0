<template> 
  <div class="payments">
    <div class="payments__content">
      ...
      <div class="payments__box" :class="{ 'payments__box--configure-link': isEnable }">
        <span v-if="isToogle" @click="$router.push({name:'payment_method-config', params: { currency: item.type } })" class="link-icon"><i class="icon-arrow-right"></i></span>
        <a v-if="isToogle" @click="$router.push({name:'payment_method-config', params:{currency:item.type}})" class="l-medium link">{{ $t('configure') }}</a>
      </div>
    ...
    <div class="payments__box payments__box--responsive-link" v-if="isEnable">
      <hr v-if="isToogle">
      <a v-if="isToogle" @click="$router.push({ name: 'payment_method-config', params: { currency: item.type } })" class="l-medium">{{ $t('configure') }}</a>
    </div>
...
const props = defineProps({
  ...
  isEnable: { //se trata de la vista principal o de la vista de 'habilitar métodos'
    type: Boolean,
    default: false,
  },
  toggle: {
