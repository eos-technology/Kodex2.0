<template>
    <section class="send">
        <h3 class="h3-bold mb-6">{{ $t('users.header') }}</h3>
        <router-link :to="{ name: 'user' }" style="text-decoration: none;" class="text-black">
            <BackDummy />
        </router-link>
        <v-card class="card-transaction pa-4 rounded-lg d-flex flex-column" v-if="!saved">
            <h3 class="h3-bold">{{ $t('user.edit-permissions') }}</h3>
        ...
        <v-card class="card-transaction pa-4 rounded-lg d-flex flex-column align-center text-center" v-if="saved">

            <div>
                <img src="@/assets/images/ready.webp" alt="">
            </div>
            <h3 class="h3-bold">{{ $t('reports.allDone') }}</h3>
            <p class="l-light mt-4">{{ $t('reports.permission') }} <span class="font-weight-bold">NombreUsuario</span>
            {{ $t('reports.permission2') }}<span class="font-weight-bold"> NombreProyecto</span></p>
            <router-link :to="{ name: 'user' }" class="w-100">
                <v-btn class="text-none text-white mt-6 w-100 primary" color="#091D8B">{{ $t('transactions.back')
                }}</v-btn>
            </router-link>
        </v-card>
    </section>
