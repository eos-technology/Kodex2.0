    ...
    <section class="reports">
        ...
        <div class="card-big reports__card" v-if="selectedMethod && !saved">
            <v-card class="pa-6" style="max-width: 450px">
                ...
                <div class="mt-6">
                    ...
                    <v-text-field placeholder="Your Wallet" variant="solo"></v-text-field>
                </div>
                <v-btn class="text-none text-white mt-4 w-100 primary" color="#091D8B" @click="saved = true">{{
                    $t('reports.generate')
                }}</v-btn>
                <router-link :to="{ name: 'user' }">
                    <v-btn variant="outlined" class="text-none mt-4 w-100 primary outlined">{{
                        $t('reports.btnCancel') }}</v-btn>
                </router-link>
            </v-card>
        </div>
        <div class="card-big reports__card d-flex flex-column align-center text-center" v-if="saved">
            <v-card class="pa-6" style="max-width: 450px">
                ...
                <p class="l-light mt-4">{{ $t('users.saveWallet') }} <span class="font-weight-bold">Tether USDT</span>
